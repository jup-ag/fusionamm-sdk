...
describe("Token Account Creation", () => {
  ...
  it("No native mint", async () => {
    const result = await prepareTokenAccountsInstructions(rpc, signer, [mintA, mintB]);
  ...
  it("Token 2022 token that requires larger token accounts", async () => {
    const result = await prepareTokenAccountsInstructions(rpc, signer, [mintTE]);
  ...
  it("Native mint and wrapping is none", async () => {
    ...
    const result = await prepareTokenAccountsInstructions(rpc, signer, [NATIVE_MINT]);
