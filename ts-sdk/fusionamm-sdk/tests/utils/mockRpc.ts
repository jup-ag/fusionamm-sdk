    ...
    _testContext = await startAnchor(
      ...
      [
        [
          ...
          new Account(BigInt(100e9), new Uint8Array(), toBytes(SYSTEM_PROGRAM_ADDRESS), false, 0n),
        ],
      ],
    );
...
export async function deleteAccount(address: Address) {
  ...
  testContext.setAccount(
    toBytes(address),
    new Account(BigInt(0), new Uint8Array(), toBytes(SYSTEM_PROGRAM_ADDRESS), false, 0n),
  );
}

export async function sendTransaction(ixs: IInstruction[]): Promise<Signature> {
...
async function mockTransport<T>(
  ...
  switch (config.payload.method) {
    ...
    case "sendTransaction":
      ...
      const signature = getBase58Decoder().decode(signatureBytes ?? new Uint8Array());
    ...
    case "getBalance":
      ...
      const accountDataForBalance = await getAccountData(addressForBalance, config.payload.params[1]);
...
export const rpc: Rpc<SolanaRpcApi> = createSolanaRpcFromTransport(mockTransport);
