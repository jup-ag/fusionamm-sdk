...
export async function prepareTokenAccountsInstructions(
  rpc: Rpc<GetAccountInfoApi & GetMultipleAccountsApi & GetMinimumBalanceForRentExemptionApi>,
  owner: TransactionSigner,
  spec: Address[] | Record<Address, bigint | number>,
): Promise<TokenAccountInstructions> {
  const mintAddresses = Array.isArray(spec) ? spec : (Object.keys(spec) as Address[]);
  ...
  if (hasNativeMint && NATIVE_MINT_WRAPPING_STRATEGY === "seed") {
    ...
    tokenAccountAddresses[NATIVE_MINT] = getAddressDecoder().decode(new Uint8Array(buffer));
