...
import {
  ...
  getOpenLimitOrderInstructionWithEphemeralSigner,
  getTickArrayAddress,
  getTickArrayMinSize,
  LimitOrder,
 ...
 * @param {TransactionSigner | Address} limitOrderMint - Ephemeral signer keypair of a limit order mint generated by generateKeyPairSigner(). In the case of a multisig transaction, it may instead be a public key retrieved via a SquadsX wallet.
...
export async function openLimitOrderInstructions(
  ...
  limitOrderMint: TransactionSigner | Address,
  poolAddress: Address,
  amount: bigint,
  ...
  const limitOrderMintAddress = typeof limitOrderMint === "string" ? limitOrderMint : limitOrderMint.address;
  ...
  const [limitOrderAddress, limitOrderTokenAccount, tickArrayAddress] = await Promise.all([
    getLimitOrderAddress(limitOrderMintAddress),
    findAssociatedTokenPda({
      owner: funder.address,
      mint: limitOrderMintAddress,
  ...
  if (typeof limitOrderMint === "string") {
    instructions.push(
      getOpenLimitOrderInstructionWithEphemeralSigner({
        funder,
        owner: funder.address,
        limitOrder: limitOrderAddress[0],
        limitOrderMint: limitOrderMintAddress,
 ...
 *
 * @param {Rpc<GetAccountInfoApi & GetMultipleAccountsApi & GetMinimumBalanceForRentExemptionApi & GetEpochInfoApi>} rpc - The Solana RPC client used to fetch token accounts and multiple accounts.
 ...
 *
 * @param {Rpc<GetAccountInfoApi & GetMultipleAccountsApi & GetMinimumBalanceForRentExemptionApi & GetEpochInfoApi>} rpc - The Solana RPC client used to fetch positions.
