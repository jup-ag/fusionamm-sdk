...
pub const FUSION_POOLS_CONFIG_DISCRIMINATOR: [u8; 8] = [191, 199, 19, 11, 75, 86, 239, 169];
...
impl<'a> TryFrom<&solana_account_info::AccountInfo<'a>> for FusionPoolsConfig {
...
pub fn fetch_all_fusion_pools_config(
  ...
  addresses: &[solana_pubkey::Pubkey],
    ...
    let mut decoded_accounts: Vec<crate::shared::DecodedAccount<FusionPoolsConfig>> = Vec::new();
    for i in 0..addresses.len() {
      let address = addresses[i];
      let account = accounts[i].as_ref()
        .ok_or(std::io::Error::new(std::io::ErrorKind::Other, format!("Account not found: {}", address)))?;
      let data = FusionPoolsConfig::from_bytes(&account.data)?;
...
pub fn fetch_all_maybe_fusion_pools_config(
  ...
  addresses: &[solana_pubkey::Pubkey],
    ...
    let accounts = rpc.get_multiple_accounts(addresses)
      .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
    let mut decoded_accounts: Vec<crate::shared::MaybeAccount<FusionPoolsConfig>> = Vec::new();
    for i in 0..addresses.len() {
      let address = addresses[i];
  ...
  #[cfg(feature = "anchor")]
  impl anchor_lang::AccountDeserialize for FusionPoolsConfig {
  ...
  #[cfg(feature = "anchor")]
  impl anchor_lang::AccountSerialize for FusionPoolsConfig {}

  #[cfg(feature = "anchor")]
  impl anchor_lang::Owner for FusionPoolsConfig {
      fn owner() -> Pubkey {
        crate::FUSIONAMM_ID
      }
  }
  ...
  impl anchor_lang::IdlBuild for FusionPoolsConfig {}

  
  #[cfg(feature = "anchor-idl-build")]
  impl anchor_lang::Discriminator for FusionPoolsConfig {
